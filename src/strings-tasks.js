...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.substring(0, indexOfFirst) +
    str.substring(indexOfFirst + value.length, str.length)
  );
}
...
function countVowels(str) {
  ...
  let s = 0;
  for (let i = 0; i < str.length; i += 1) {
    s += vowels.includes(str.at(i)) ? 1 : 0;
  }
  return s;
}
/**
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const tmp = str.at(i);
    arr.push(tmp === tmp.toLowerCase() ? tmp.toUpperCase() : tmp.toLowerCase());
  }
  return arr.join('');
}
